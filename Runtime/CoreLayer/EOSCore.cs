...
namespace RobProductions.OpenEOS
{
	public static class EOSCore
	{
		...
		public static Result Login(PlatformInterface platformSession)
		{
			var authInterface = platformSession.GetAuthInterface();
			if (authInterface == null)
			{
				LogEOS("Warning: Could not retrieve AuthInterface from provided platformSession");
		...
		/// <summary>
		/// Use this to set whether or not EOSCore should log its own warnings.
		/// </summary>
		/// <param name="v"></param>
		...
		private static void LogEOS(string message)
		{
			if(logWarnings)
			{
